...
import 'package:maxeyfresh/StyleDecoration/styleDecoration.dart';
...
import 'package:maxeyfresh/model/InvoiceModel.dart';
import 'package:maxeyfresh/model/OrderDliverycharge.dart';
                          ...
                          children: <Widget>[
                            Padding(
                              ...
                              child: Text(
                                ...
                                style: CustomTextStyle.textFormFieldMedium.copyWith(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold),
                              ),
                            ),
                            codp == 'yes'
                                ? Expanded(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: [
                  createAddressText(widget.time != null ? widget.time : '0:0', 6),
                  SizedBox(
                    width: 20,
                  ),
                  createAddressText(widget.date != null ? widget.date : '0:0', 6),
                ],
              ),
              ...
              RichText(
                text: TextSpan(children: [
                  TextSpan(text: "Mobile : ", style: CustomTextStyle.textFormFieldMedium.copyWith(fontSize: 12, color: Colors.grey.shade800)),
                  TextSpan(
    ...
    return Container(
      child: Row(
        children: <Widget>[
          ...
          FlatButton(
            onPressed: () {},
            child: Text("Add New Address", style: CustomTextStyle.textFormFieldSemiBold.copyWith(fontSize: 12, color: Colors.indigo.shade700)),
            splashColor: Colors.transparent,
            highlightColor: Colors.transparent,
          ),
          Spacer(
  ...
  checkoutItem() {
    return Container(
      ...
      child: Card(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(4))),
        child: Container(
          decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(4)), border: Border.all(color: Colors.grey.shade200)),
...
//              padding: EdgeInsets.only(right: 8, top: 4),
          child: Container(
            ...
            child: Text(
              prodctlist1[position].pname == null ? 'name' : prodctlist1[position].pname,
              maxLines: 2,
              softWrap: true,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                    ...
                    image: DecorationImage(
                        fit: BoxFit.cover,
                        image: NetworkImage(
                          Constant.Product_Imageurl + prodctlist1[position].pimage,
                        ))),
              ),
              Expanded(
                child: Container(
                  ...
                  Flexible(
                    child: Padding(
                      ...
                      child: TextFormField(
                        ...
                        decoration: const InputDecoration(hintText: "Apply Coupon Code"),
//                        enabled: !_status,
                      ),
                    ),
//                    flex: 2,
...
//                    flex: 2,
              ...
              Text(
                "PRICE DETAILS",
                style: CustomTextStyle.textFormFieldMedium.copyWith(fontSize: 12, color: Colors.black, fontWeight: FontWeight.w600),
              ),
              SizedBox(
                height: 4,
              ),
...
//    print("123  Unable to generate Employee Invoice");
  ...
  Future _uploadProducts(String invoice, String paytype) async {
    ...
    for (int i = 0; i < prodctlist1.length; i++) {
      ...
      map['price'] = (int.parse(prodctlist1[i].costPrice) * prodctlist1[i].pQuantity).toString();
      ...
      try {
        ...
        if (response.statusCode == 200) {
//        final jsonBody = json.decode(response.body);
          ProductAdded1 user = ProductAdded1.fromJson(jsonDecode(response.body));

          setState(() {
            if (user.success.toString() == "true" && i == (prodctlist1.length - 1) && paytype == 'ONLINE') {
            ...
            } else if (user.success.toString() == "true" && i == (prodctlist1.length - 1) && paytype == 'COD') {
              showLongToast(' Your  order is  sucessfull');
              dbmanager.deleteallProducts();
              Constant.itemcount = 0;
              Constant.carditemCount = 0;
            ...
            } else if (user.success.toString() == "true" && i == (prodctlist1.length - 1) && paytype == 'UPI/QRCODE') {
              showLongToast(' Your  order is  sucessfull');
              dbmanager.deleteallProducts();
              Constant.itemcount = 0;
              Constant.itemcount = 0;
...
//
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => ShowInVoiceId1(user.Invoice)),
              );
            } else if (user.success.toString() == "true" && i == (prodctlist1.length - 1) && paytype == 'THROUGH ACCOUNTS') {
              showLongToast(' Your  order is  sucessfull');
              dbmanager.deleteallProducts();
              Constant.itemcount = 0;
              Constant.itemcount = 0;
...
//          openCheckout();
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => ShowInVoiceId2(user.Invoice)),
              );
            } else {
  ...
  Future _afterPayment(String orderid, String signature, String paymentId) async {
    ...
    final response = await http.post(Constant.base_url + 'verifyUser.php', body: map);
        ...
        children: <Widget>[
          ...
          Column(
            ...
            children: <Widget>[
              Text(
                fast_text,
                style: CustomTextStyle.textFormFieldMedium.copyWith(color: Colors.black, fontSize: 14, fontWeight: FontWeight.w600),
              ),
              SizedBox(
                height: 5,
              ),
